...
import { BookingWithStagiaire } from "../../_types";
      ...
      {isLoadingDetails ? (
        ...
        <div className="space-y-2">
          {bookings.map((booking) => (
            <div key={booking.id} className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3">
              <div className="space-y-0.5">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-400 pt-0.5">
                  {booking.stagiaire?.weight != null && <span>{booking.stagiaire.weight} kg</span>}
                  {booking.stagiaire?.height != null && <span>{booking.stagiaire.height} cm</span>}
                </div>
              </div>
