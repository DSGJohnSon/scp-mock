...
export function ExportClientsDialog({
  ...
  const activeDeptCodes = useMemo(() => {
    const clients = (allData?.clients as unknown as ClientWithOrders[]) || [];
  ...
  const currentSelectionData = useMemo(() => {
    const dataSource =
      exportQuantity === "page"
        ? currentPageClients
        : (allData?.clients as unknown as ClientWithOrders[]) || [];
    ...
    const csvRows = currentSelectionData.map((client) => {
      return selectedFields
        .map((id) => {
          ...
          else val = (client as Record<string, unknown>)[id] || "n/a";
