...
import { EditedStageState, StageWithDetails } from "./stage-types";
    ...
    <div>
      ...
      {isEditing && editedStage ? (
        <div className="space-y-2">
          <Label htmlFor="moniteurs" className="text-sm">
            Sélectionner des moniteurs
          </Label>
          <MultiSelect
            options={
              moniteurs?.map((m) => ({
          ...
          {displayStage.moniteurs.map((md) => (
            ...
            >
              <Avatar className="h-8 w-8">
                <AvatarImage
                  src={md.moniteur.avatarUrl ?? undefined}
                  alt={md.moniteur.name}
                />
