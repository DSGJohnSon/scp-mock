...
export function ExportStagiairesDialog({
  ...
  const currentSelectionData = useMemo(() => {
    ...
    return dataSource.filter((stagiaire) => {
      const totalRes = stagiaire.stageBookings.length;
      ...
      if (afterDate) {
        const dates = stagiaire.stageBookings.map((b) => new Date(b.createdAt));
        if (dates.length > 0) {
  ...
  const handleExport = async () => {
    ...
    const csvRows = currentSelectionData.map((stagiaire) => {
      ...
      const bookingDates = stagiaire.stageBookings.map((b) => new Date(b.createdAt));
      ...
      return selectedFields
        .map((id) => {
          ...
          if (id === "totalBookings") val = totalStages;
            ...
            val = stagiaire.birthDate
              ...
              : "n/a";
          else val = (stagiaire as Record<string, unknown>)[id] || "n/a";
